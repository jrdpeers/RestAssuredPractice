...
public class SwapiAPIUsageTest {
    ...
    // Use Swapi object to fetch a person
    @Test
    public void canGetDarthAndParseIntoObject(){
    	Swapi swapi = new Swapi();
    	Person darth = swapi.getPerson(4);
    	Assert.assertEquals(202, darth.height);
    }
    
    // Use Swapi object to fetch a planet
    @Test
    public void canGetHothAndParseIntoObject(){
    	Swapi swapi = new Swapi();
    	Planet hoth = swapi.getPlanet(4);
    	Assert.assertEquals("tundra, ice caves, mountain ranges", hoth.terrain);
    }
    
    // Use Swapi object to fetch a starship
    @Test
    public void canGetXWingAndParseIntoObject(){
    	Swapi swapi = new Swapi();
    	Starship xwing = swapi.getStarship(12);
    	Assert.assertEquals(12.5, xwing.length, 0);
    	Assert.assertEquals(4, xwing.pilots.length);
    }
    
    // Use Swapi object to fetch a vehicle
    @Test
    public void canGetSandCrawlerAndParseIntoObject(){
    	Swapi swapi = new Swapi();
    	Vehicle sandcrawler = swapi.getVehicle(4);
    	Assert.assertEquals("Corellia Mining Corporation", sandcrawler.manufacturer);
    }
